package ua.syt0r.kanji.screen.common
...
@Composable
fun test(strokes: List<Path>) {
    val animatedStokeIndex = mutableStateOf(0)

    Button(onClick = {
        Log.d("123", "click")
        animatedStokeIndex.value = (animatedStokeIndex.value + 1) % strokes.size
    }) {

        AnimatedKanji(
            modifier = Modifier.size(200.dp)
                .background(Color.Green),
            animateStroke = animatedStokeIndex.value,
            strokes = strokes
        )
