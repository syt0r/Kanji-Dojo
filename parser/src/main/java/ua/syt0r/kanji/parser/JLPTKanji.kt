...
private val map: Map<Set<Char>, CharactersClassification.JLPT> = listOf(n5, n4, n3, n2, n1)
    .zip(CharactersClassification.JLPT.values())
    .associate { it.first.toSet() to it.second }

fun findJLPTForKanji(kanji: Char): CharactersClassification.JLPT? {
    return map.firstNotNullOfOrNull { (chars, jlpt) ->
        jlpt.takeIf { chars.contains(kanji) }
    }
}